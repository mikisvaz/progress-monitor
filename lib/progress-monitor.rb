     ...
     def monitorable1(method_name)
       module_eval{ 
         ...
         define_method(method_name) do |&block|
           if Progress.monitor?
             progress_meter = Progress.add_progress_meter(self.monitor_size)
             monitor_step = nil unless defined? monitor_step
             res = self.send(orig_name.to_sym) {|v| progress_meter.tick(monitor_step); block.call(v) }
     ...
     def monitorable2(method_name)
       module_eval{ 
         ...
         if Progress.monitor?
           progress_meter = Progress.add_progress_meter(self.monitor_size)
           monitor_step = nil unless defined? monitor_step
           res = self.send(orig_name.to_sym) {|v1,v2| progress_meter.tick(monitor_step); block.call(v1,v2) }
           Progress.remove_last_meter
           res
         else
           self.send(orig_name.to_sym) {|v1,v2| bloc.call(v1,v2) }
         end
         end
       }
     end
